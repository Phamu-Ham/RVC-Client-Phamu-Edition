...
from tools.runtime_support import debugt
...
class RVC:
    def __init__(
        ...
        try:
            ...
            # global config
            self.config = config
            self.harvest_pool = harvest_pool or (
                last_rvc.harvest_pool if last_rvc is not None else HarvestPool()
            )
            self.inp_q = inp_q
            self.opt_q = opt_q
            # device="cpu"########强制cpu测试
            ...
            self.index_warning = ""
            if index_rate != 0:
                self._ensure_index_loaded(last_rvc)
            ...
            # Let the caller report a load failure, not a half-initialized RVC.
            raise
    ...
    def _ensure_index_loaded(self, previous=None):
        path = os.path.normcase(os.path.abspath(self.index_path))
        stat = os.stat(path)
        signature = (path, stat.st_size, stat.st_mtime_ns)
        source = previous if previous is not None else self
        if (getattr(source, "_index_signature", None) == signature
                and hasattr(source, "index")):
            self.index, self.big_npy = source.index, source.big_npy
        else:
            # FAISS' filename API is not Unicode-safe in some Windows builds.
            if str(self.index_path).isascii():
                index = faiss.read_index(self.index_path)
            else:
                index = faiss.deserialize_index(np.fromfile(self.index_path, dtype=np.uint8))
            if hasattr(index, "nprobe"):
                index.nprobe = min(8, index.nlist)
            big_npy = index.reconstruct_n(0, index.ntotal)
            self.index, self.big_npy = index, big_npy
    ...
    def get_f0(self, x, f0_up_key, n_cpu, method="harvest"):
        ...
        if n_cpu == 1:
            ...
            f0 *= pow(2, f0_up_key / 12)
            return self.get_f0_post(f0)
        f0bak = self.harvest_pool.compute(x, n_cpu)
        ...
        try:
            if hasattr(self, "index") and self.index_rate != 0:
                ...
                if not np.isfinite(score).all() or (score <= 0).any():
                    raise ValueError("Indexの距離が無効です")
                if (ix >= 0).all():
                    ...
                    self.index_warning = "Indexに有効な検索データがありません。added形式のIndexを選んでください。"
            else:
                self.index_warning = ""
                debugt("Index search FAILED or disabled")
        except Exception as error:
            warning = "Index検索失敗：Indexを選び直すかIndex Rateを0にしてください。"
            if getattr(self, "index_warning", "") != warning:
                printt("Index search FAILED: %s", error)
